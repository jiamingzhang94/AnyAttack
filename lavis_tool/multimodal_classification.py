...
class MultimodalClassificationTask(BaseTask):
    ...
    def valid_step(self, model, samples):
        ...
        results = []
        cfg = registry.mapping["state"]["configuration"]
        # instruct_class_label = {0: "no", 1: 'maybe', 2: "yes"}
        instruct_class_label = {"no": 0, "maybe": 1, "yes": 2}
        # argspec = inspect.getargspec(model.predict_answers)
        ...
        #                             )
        if cfg.config.model.arch == "albef_classification":
            outputs = model.predict(samples)
            predictions = outputs["predictions"]
            predictions = predictions.max(1)[1].cpu().numpy()
        elif "instruct" in cfg.config.model.arch:
            predictions = model.predict_class(samples, candidates=['no', 'maybe', 'yes'], n_segments=self.segments)
            predictions = predictions[:, 0]
            outputs = {"predictions": predictions, 'targets': samples["label"]}

            # predictions = [instruct_class_label[i] for i in predictions]
            # predictions = model.generate(samples)
            # outputs = {"predictions": predictions, 'targets': samples["label"]}

        else:
            raise Exception(f"Model does not support model {cfg.config.mode}")
